...
#' @import ggplot2
#' @export
theme_chemistry <- function(base_size = 14, base_family = "",
                          line_size = 0.5, ...) {
  ...
  theme_bw(base_size = base_size, base_family = base_family, ...) %+replace%
    theme(
      ...
      axis.text.x = element_text(margin = ggplot2::margin(t = small_size/4),
                                 vjust = -1.2),
      axis.text.y = element_text(margin = ggplot2::margin(r = small_size),
                                 hjust = -1),
      ...
      panel.border = element_rect(colour = "black", fill=NA, size=line_size),
