...
chemplot <- function (x, y, log = "", main = "", side = 1:2, majorn = 5,
          ...
          mgp = c(2, 0.5, 0), mtline = 2, xlab = "", ylab = "", crunch = TRUE,
          ...
          pch = 16, col = '#592C88', ...)
{
  if (class(x)[1] == "histogram") {
    ...
    do.call("magicaxis::maghist", c(list(x = x, xlim = xlim, ylim = ylim,
                              ...
                              usepar = usepar, grid = grid, grid.col = grid.col,
                              grid.lty = grid.lty, grid.lwd = grid.lwd), dots))
  }
  else {
    if (missing(y)) {
      if (is.data.frame(x) | is.matrix(x)) {
        if (ncol(x) > 2) {
          ...
          if (length(xlim) == 1) {
            sel = !is.na(x[, 1]) & !is.nan(x[, 1]) &
              !is.null(x[, 1]) & is.finite(x[, 1])
            if (log[1] == "x" | log[1] == "xy" | log[1] ==
                "yx") {
              sel = sel & x[, 1] > 0
            }
          ...
          if (length(ylim) == 1) {
            sel = !is.na(x[, 2]) & !is.nan(x[, 2]) &
              !is.null(x[, 2]) & is.finite(x[, 2])
            if (log[1] == "y" | log[1] == "xy" | log[1] ==
                "yx") {
              sel = sel & x[, 2] > 0
            }
          ...
          plot(x = x, axes = FALSE, xlab = "", ylab = "",
               ...
               panel.first = if (side[1] != FALSE | axes ==
                 ...
                 magicaxis::magaxis(side = side, majorn = majorn, minorn = minorn,
                         ...
                         logpretty = logpretty, prettybase = prettybase,
                         powbase = powbase, hersh = hersh, family = family,
      ...
      if (length(xlim) == 1) {
        sel = !is.na(x) & !is.nan(x) & !is.null(x) &
          is.finite(x)
        if (log[1] == "x" | log[1] == "xy" | log[1] ==
            "yx") {
          sel = sel & x > 0
